...
type CheckoutView = 'form' | 'processing' | 'success' | 'failed'

interface CheckoutPageProps {
  onNavigateBack: () => void
}

export const CheckoutPage: React.FC<CheckoutPageProps> = ({ onNavigateBack }) => {
  const { activeProduct, closeOrderModal } = useOrder()
  ...
  const handleGoBack = useCallback(() => {
    closeOrderModal()
    onNavigateBack()
  }, [closeOrderModal, onNavigateBack])
      ...
      <header className="sticky top-0 z-40 border-b border-border/60 bg-[#0A0E17]/95 backdrop-blur-lg">
        <div className="mx-auto max-w-4xl flex items-center justify-between px-4 sm:px-6 py-3.5">
          ...
          >
            ...
            <span className="font-display font-bold text-sm tracking-wide text-cream group-hover:text-cyan-signal transition-colors">
              Back to BoneTalk
            </span>
          </button>
          ...
          <div className="relative rounded-sm border border-cyan-signal/20 bg-[#0A0E17] shadow-[0_25px_70px_rgba(0,0,0,0.6),0_0_40px_rgba(0,216,165,0.08)] overflow-hidden">
            ...
            <div className="p-6 sm:p-10 space-y-8">
              ...
              {errorMessage && view !== 'success' && (
                <div className="flex items-start gap-3 rounded-sm border border-red-500/30 bg-red-500/10 p-4 text-xs text-red-300 font-mono animate-in fade-in">
                ...
                <form onSubmit={handlePlaceOrderAndPay} noValidate className="space-y-8">
                  ...
                  <div className="rounded-sm border border-border/80 bg-graphite/60 p-5 sm:p-7">
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5">
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="font-display text-xl sm:text-2xl font-bold text-cream truncate">
                          {activeProduct?.name || 'BoneTalk Wearable'}
                        </h3>
                        <p className="text-xs sm:text-sm text-cream-muted line-clamp-2 mt-1">
                          {activeProduct?.benefit || 'Surface EMG assistive communication device.'}
                        </p>
                        <div className="mt-3 text-cyan-signal font-mono text-base sm:text-lg font-semibold">
                          ₹{unitPrice.toLocaleString('en-IN')}
                          <span className="text-[10px] sm:text-xs text-cream-muted font-normal ml-2">/ unit</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3 border border-border rounded-sm bg-graphite-light/50 px-3 py-2 sm:self-center">
                        <button
                          ...
                          className="h-7 w-7 flex items-center justify-center text-cream-muted hover:text-cyan-signal disabled:opacity-30 cursor-pointer rounded-sm hover:bg-graphite-elevated transition-colors"
                  ...
                  <div className="space-y-5">
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                      {/* Full Name */}
                      <div>
                        <label htmlFor="customer-name" className="block font-mono text-[10px] uppercase tracking-wider text-cream-muted mb-1.5">
                          Customer Name *
                        </label>
                        <input
                          id="customer-name"
                      ...
                      {/* Mobile Number */}
                      <div>
                        <label htmlFor="customer-phone" className="block font-mono text-[10px] uppercase tracking-wider text-cream-muted mb-1.5">
                      ...
                      <div className="sm:col-span-2">
                        <label htmlFor="customer-email" className="block font-mono text-[10px] uppercase tracking-wider text-cream-muted mb-1.5">
                      ...
                      {/* Delivery Address */}
                      <div className="sm:col-span-2">
                        <label htmlFor="customer-address" className="block font-mono text-[10px] uppercase tracking-wider text-cream-muted mb-1.5">
                      ...
                      {/* City */}
                      <div>
                        <label htmlFor="customer-city" className="block font-mono text-[10px] uppercase tracking-wider text-cream-muted mb-1.5">
                          City *
                        </label>
                        <input
                          id="customer-city"
                      ...
                      {/* State */}
                      <div>
                        <label htmlFor="customer-state" className="block font-mono text-[10px] uppercase tracking-wider text-cream-muted mb-1.5">
                          State *
                        </label>
                        <input
                          id="customer-state"
                      ...
                      {/* PIN Code */}
                      <div className="sm:col-span-2">
                        <label htmlFor="customer-pincode" className="block font-mono text-[10px] uppercase tracking-wider text-cream-muted mb-1.5">
                          PIN Code (6 Digits) *
                        </label>
                        <input
                          id="customer-pincode"
                  ...
                  <div className="rounded-sm border border-cyan-signal/30 bg-cyan-signal/[0.03] p-5 sm:p-6">
                    ...
                    <div className="flex items-center justify-between p-3.5 rounded-sm border border-cyan-signal/40 bg-[#06080B]/80 mt-2">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <span className="font-bold text-sm text-cream block">Razorpay Gateway</span>
              ...
              {view === 'success' && verifiedOrder && (
                ...
                >
                  <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-emerald-500/20 border border-emerald-400 text-emerald-400 shadow-[0_0_30px_rgba(16,185,129,0.3)] animate-pulse">
                  ...
                  <div className="rounded-sm border border-border/80 bg-graphite-elevated/70 p-6 sm:p-8 text-left font-mono text-sm space-y-4 shadow-inner max-w-xl mx-auto">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-cyan-signal font-bold text-base">
              ...
              {view === 'failed' && (
                ...
                >
                  <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-red-500/20 border border-red-500/40 text-red-400 shadow-[0_0_25px_rgba(239,68,68,0.25)]">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 pt-2 max-w-md mx-auto">
                    ...
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-2 rounded-sm border border-border hover:border-cyan-signal text-cream font-mono py-3.5 px-6 text-sm uppercase tracking-wider transition-colors cursor-pointer"
                    >
