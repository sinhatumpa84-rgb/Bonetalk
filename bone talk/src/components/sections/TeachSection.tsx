...
export function TeachSection() {
  ...
  // Active command selected for Voice Output inspection & playback
    ...
    >
      ...
      <div className="mx-auto max-w-[1400px] px-6 md:px-10">
        <div className="grid grid-cols-1 items-start gap-16 lg:grid-cols-2">
          {/* Left Column: Information & Vocabulary Selection */}
          <div>
            ...
            >
              ...
              its machine learning pipeline to your specific muscle gestures in under 60 seconds — and transforms them into clear, audible speech.
            </motion.p>

            {/* Saved Trained Phrases List */}
            <div className="mt-10 border-t border-border pt-6">
              <div className="flex items-center justify-between mb-4">
                <span className="block font-mono text-[10px] tracking-[0.25em] text-cream-muted uppercase">
                  Active Trained Vocabulary ({savedCommands.length})
                </span>
                <span className="font-mono text-[9px] text-cyan-signal">
                  Click to inspect &amp; play voice
                </span>
              </div>
                ...
                {savedCommands.map((cmd) => {
                  ...
                  return (
                    <button
                      ...
                      className={`inline-flex items-center gap-2 rounded-sm border px-3 py-2 font-mono text-xs transition-all cursor-pointer ${
                        isSelected
                          ? 'border-cyan-signal bg-cyan-signal/[0.12] text-cream shadow-sm scale-102'
                    ...
                    >
                      ...
                      <span className="font-bold">{cmd.phrase}</span>
          ...
          {/* Right Column: Training Suite & Dynamic Voice Output Panel */}
          <div className="space-y-6">
                  ...
                  <div className="mt-6 flex flex-col gap-3 w-full max-w-sm sm:mt-8 sm:gap-4">
                    ...
                    <form onSubmit={handleCustomSubmit} className="flex flex-col gap-2 sm:flex-row">
                      ...
                      <button
                        ...
                        className="rounded-sm border border-cyan-signal/40 bg-cyan-signal/[0.1] px-4 py-2 font-mono text-xs text-cyan-signal transition-colors hover:bg-cyan-signal/[0.2] disabled:opacity-40 min-h-[44px] flex items-center justify-center font-bold"
                      >
                        TRAIN
                      </button>
                    </form>
                  ...
                  >
                    <div className="mb-4 flex items-center justify-between border-b border-border pb-3 sm:mb-6 sm:pb-4">
                      ...
                      {phase === 'learned' && (
                        <span className="font-mono text-[9px] sm:text-[10px] text-medical uppercase font-bold flex items-center gap-1">
                          <CheckCircle2 size={13} /> PATTERN LEARNED ✓
                        </span>
                      )}
                    </div>
                    ...
                    {phase === 'learned' && (
                      <motion.div
                        ...
                        className="space-y-4"
                      >
                        {/* Voice Output Ready Card integrated upon learning */}
                        <VoiceOutputPanel
                          command={commandText}
            ...
            {/* Secondary Voice Output Panel for Active Selected Vocabulary if not currently in active calibration */}
            {phase === 'idle' && (
              <VoiceOutputPanel
