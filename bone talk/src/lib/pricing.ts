// ─── CENTRALIZED BONETALK PRICING SOURCE OF TRUTH ────────────────────────────
// Core Rules:
...
export interface BoneTalkPriceResult {
  numeric: number
  display: string
  paise: number
  currency: 'INR'
 ...
 * 
 * @param rawInput - Raw price number or string (e.g., 12799, 13742, "₹14,000")
 * @returns BoneTalkPriceResult with guaranteed XX99 ending within [12900, 15799]
 */
export function calculateBoneTalkPrice(rawInput?: number | string | null): BoneTalkPriceResult {
  let num: number
 ...
 */
export function formatBoneTalkPrice(amount: number): string {
